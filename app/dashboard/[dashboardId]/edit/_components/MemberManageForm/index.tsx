...
import { usePaginationGetDashboardMemberList } from '../../_hooks/usePaginationGetDashboardMemberList.query';
...
const MemberManageForm = ({ dashboardId }: MemberManageFormProps) => {
  ...
  const { data, isFetched, isSuccess, hasNextPage, hasPreviousPage, fetchNextPage, fetchPreviousPage, isPending } =
    usePaginationGetDashboardMemberList({ dashboardId, size: sizePerPage, currentPage });
  ...
  const slicedMemberList = memberList.slice((currentPage - 1) * sizePerPage, currentPage * sizePerPage);
  ...
  return (
    <S.Form>
      ...
      <MemberListTable memberList={slicedMemberList} />
    </S.Form>
  );
};
